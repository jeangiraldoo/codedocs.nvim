...
vim.api.nvim_create_user_command("Codedocs", function(opts)
	...
	if choice and not vim.tbl_contains(require("codedocs.annotation_builder").get_annot_list(), choice) then
		vim.notify("Invalid option: " .. (choice or "nil"), vim.log.levels.ERROR)
		return
	end
	require("codedocs").generate(choice and { annot_name = choice })
end, {
	nargs = "?",
	complete = function(arglead)
		return vim.tbl_filter(
			function(opt) return opt:find(arglead) == 1 end,
			require("codedocs.annotation_builder").get_annot_list()
		)
	end,
})
